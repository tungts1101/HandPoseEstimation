    ...
    for epoch in range(int(cur_state[str(fold)]['epoch']), args.epoch + 1):
        ...
        for i, data in enumerate(tqdm(train_dataloader, 0)):
            ...
            if isinstance(network, CascadedNetworkObj):
                ...
                if args.contain_obj:
                    loss = criterion(estimation * 1000, torch.cat((gt_xyz, obj_xyz.reshape(-1, 24)), dim=1) * 1000)
                    ...
                    loss = criterion(estimation * 1000, gt_xyz * 1000)
            ...
            with torch.no_grad():
                for i, data in enumerate(tqdm(test_dataloader, 0)):
                    ...
                    if isinstance(network, CascadedNetworkObj):
                        ...
                        obb_len = torch.diff(bound_obb, dim=1)
                        if args.contain_obj:
                            eval_loss = criterion(estimation * 1000, torch.cat((gt_xyz, obj_xyz.reshape(-1, 24)), dim=1) * 1000)
                            # eval_loss = 0.9 * criterion(estimation[:, :63].reshape(-1, 21, 3) * obb_len, gt_xyz.reshape(-1, 21, 3) * obb_len) + \
                            #     0.1 * criterion(estimation[:, 63:].reshape(-1, 8, 3) * obb_len, obj_xyz.reshape(-1, 8, 3) * obb_len)
                        else:
                            eval_loss = criterion(estimation * 1000, gt_xyz * 1000)
